...
`m14/freetext-ap`, tree clean). **GREEN landed 2026-08-08 (commits `507ff443` +
`56536fe7`; see "GREEN VERIFIED" at the bottom of this file).** Use this file as
the implementation contract; do NOT re-derive.
...
## GREEN VERIFIED (2026-08-08, commits `507ff443` + `56536fe7`)

- WIP `a6ce8e87` built CLEAN on first attempt (Release/Ninja) — zero compile
  fixes. The GOTCHA was avoided by plumbing: font bytes are set at runtime via
  `setRtlFreeTextFontData(QByteArray)` (the test reads the TTF), so core needs
  no `target_compile_definitions`. Do NOT add TEST_FONT_ARABIC to
  `src/Pdf4QtLibCore/CMakeLists.txt`.
- RED test FAILED with a CORRECT output PDF — the test walk, not the impl, was
  wrong. Byte-inspection of out.pdf: AP /N = form stream with
  `/Resources << /Font << /F2 8 0 R >> >>`; object 8 = `/Subtype /Type0` +
  `/DescendantFonts [ << /Subtype /CIDFontType2 ... /FontDescriptor << ...
  /FontFile2 5 0 R >> >> ]`. **PITFALL: `/FontDescriptor` is NOT a direct key of
  the Type0 dict — it lives inside `DescendantFonts[0]` (CIDFontType2
  descendant). An AP walk asserting "Type0 + FontFile2" must descend
  `/DescendantFonts[0]` → `/FontDescriptor` → `/FontFile2`.** Fix landed in the
  TEST walk (`507ff443`); assertion semantics unchanged.
- Authored test file was NEVER clang-format-clean (RED commit violated the gate
  at 2 spots); fix commit carried a whole-file `clang-format -i`. Run the format
  check before committing RED tests.
- LTR path byte-identical confirmed: WIP diff purely additive (0 deletions in
  pdfdocumentbuilder.cpp); baseline 16/16 unchanged. Final suite 17/17 green;
  REPO_MAP.md regenerated byte-identical (no commit needed).
